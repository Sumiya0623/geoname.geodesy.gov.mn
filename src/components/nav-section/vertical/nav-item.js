  ...
  (
    ...
    const renderIcon = () => {
      if (depth === 1) {
        return (
          <SvgColor
            src={`/assets/icons/navbar/${
              typeof icon === "string" ? icon : "ic_folder"
            }.svg`}
      ...
      if (typeof icon === "string") {
        return <Iconify icon={icon} width={24} />;
      }
      if (icon) {
        return icon;
      }
      return <Iconify icon="material-symbols:folder" width={24} />;
    };
...
const StyledNavItem = styled(ListItemButton, {
  ...
  return {
    ...baseStyles.item,
    minHeight: subItem ? 36 : 44,
    "& .icon": baseStyles.icon,
    "& .sub-icon": {
      ...baseStyles.icon,
      display: subItem ? "flex" : "none",
      alignItems: "center",
