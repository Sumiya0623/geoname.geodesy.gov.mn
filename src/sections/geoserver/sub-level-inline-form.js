...
export default function SubLevelInlineForm({
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const request_body = { ...rest, parent_id: effectiveParent || null };
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        enqueueSnackbar(
          `Store амжилттай ${currentConstant ? "өөрчлөгдлөө" : "нэмэгдлээ"}`,
        );
      ...
      enqueueSnackbar(
        `Store ${currentConstant ? "өөрчлөх" : "үүсгэх"} үед алдаа гарлаа`,
        {
          variant: "error",
        },
      );
