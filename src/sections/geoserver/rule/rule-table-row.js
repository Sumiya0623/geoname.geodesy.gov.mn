...
export default function StyleTableRow({
  ...
  const { id, name, filters, fill_color, size, symbolizer, icon } = row;
  ...
  return (
    <>
      <TableRow>
            ...
            <Stack direction="row" flexWrap="wrap" gap={1}>
              {filters.map((f, i) => (
                <Chip
                  key={f.id ?? i}
                  size="small"
                  label={`${f.field ?? f.name ?? "?"} ${OP_LABELS[f.operator] ?? f.operator} ${f.value ?? ""}`}
                  variant="outlined"
                />
              ))}
            </Stack>
        ...
        <TableCell>{fill_color || "unkn"}</TableCell>
        <TableCell>{size || "unkn"}</TableCell>
      ...
      <Dialog open={form.value} onClose={form.onFalse} fullWidth maxWidth="md">
        <DialogTitle>Style засах</DialogTitle>
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Та <strong>{name}</strong> style-ийг устгахдаа итгэлтэй байна уу?
          </>
        }
        action={
