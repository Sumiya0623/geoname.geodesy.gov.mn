...
export default function RuleNewEditForm({
  ...
  const defaultValues = useMemo(
    () => buildDefaults(currentRule, layer),
    [currentRule, layer],
  );
  ...
  useEffect(() => {
    reset(buildDefaults(currentRule, layer));
    if (!currentRule?.render_mode) {
      setValue("render_mode", "symbol", { shouldDirty: false });
    }
  }, [currentRule, layer, reset, setValue]);

  const symbolizer = layer.table.code;
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      // === 0) Нийтлэг талбарууд ===
      fd.append("layer_id", layer?.id);
      fd.append("name", asStr(data.name?.trim()));
      fd.append("render_mode", mode); // backend салаалтад ашиглана

      // Масштаб / ил харагдалт / эрэмбэ
      fd.append("min_scale_denom", asNum(data.min_scale_denom));
      fd.append("max_scale_denom", asNum(data.max_scale_denom));
      fd.append("is_visible", asBool(!!data.is_visible));
      ...
      switch (mode) {
        case "text": {
          ...
          // point/line/polygon талбар НЭМЭХГҮЙ
          break;
        }

        case "symbol": {
          // ЗӨВХӨН GEOMETRY SYMBOL
          const baseSym = data.symbolizer || layer?.table?.code || "polygon";
          fd.append("symbolizer", asStr(baseSym));
          fd.append("is_with_text", "false");
          ...
          // point options (зөвхөн point үед ашиглагдана, backend талдаа үл тоож болно)
      ...
      if (res?.status === 200 || res?.status === 201) {
        enqueueSnackbar(
          `Rule амжилттай ${currentRule ? "хадгалагдлаа" : "үүсгэлээ"}`,
        );
        onCloseForm?.();
        refetch?.();
      }
    } catch (err) {
      enqueueSnackbar("Алдаа гарлаа", { variant: "warning" });
    }
  });
