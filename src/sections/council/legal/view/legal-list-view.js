...
import { useGetLegalOrders, useGetLegalLevels } from "src/api/legal";
...
// түвшнүүдийг (types = LEGAL_LEVELS, LegalOrder.govlevel) татаж chip болгоно; chip сонгоход
// тэр түвшингээр нарийсгана. Нэмэхэд LegalOrder‑г төсөлд (projectId) холбож хадгална.
// ----------------------------------------------------------------------
...
const TABLE_HEAD = [
  ...
  { id: "name", label: "Нэр" },
  { id: "", label: "Нэгж" },
  // Байгууллага (LEGAL_ORGS) — зөвхөн аймаг (1x) / сум (2x) түвшний
  // шийдвэрт бөглөгддөг; бусад мөрөнд «-» болно.
...
export default function LegalListView({
  ...
  // Chip‑ийн түвшнүүд (LegalOrder.govlevel — «Дээд тогтоол»):
  //  - projectId БАЙВАЛ → тухайн төсөлд бүртгэгдсэн түвшнүүд (registered_types).
  //  - projectId БАЙХГҮЙ (ж: /dashboard/legal) → бүх LEGAL_LEVELS (ерөнхий сан).
  const { champaign, champaignMutation } = useGetChampaign(projectId);
  const { legalLevels, legalLevelsMutation } = useGetLegalLevels();
  const types = useMemo(
    () => byCode(projectId ? champaign?.registered_types || [] : legalLevels),
    [projectId, champaign?.registered_types, legalLevels],
  );
  ...
  // бүх орд. Түвшин (chip) сонгосон бол govlevel‑ээр нарийсгана.
  const requestBody = useMemo(
    () => ({
      ...
      ...(selectedId ? { govlevel: selectedId } : {}),
  ...
  const refreshAll = useCallback(() => {
    ...
    } else if (legalLevelsMutation) {
      legalLevelsMutation();
    }
  }, [legalOrdersMutation, projectId, champaignMutation, legalLevelsMutation]);
