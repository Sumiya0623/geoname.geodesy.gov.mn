...
export default function MayagtView({
  ...
  const [qNomek, setQNomek] = useState(""); // нэрлэвэрийн тусдаа хайлт
  ...
  }, [tab, q, qNomek, sum, aimag, typeFilterId]);

  const cols = COLS[tab];

  // Хайлтаар шүүсэн мөрүүд — 2 талбар ЗЭРЭГ (AND) үйлчилнэ:
  //  • q      — тухайн маягтын ХАРАГДАЖ БУЙ бүх багана (нэр, зураг дээрх нэр, ...)
  //  • qNomek — зөвхөн нэрлэвэрийн код (1:25000 / 1:100000)
  // Латин/кирилл хольцыг fold()-оор жигдрүүлнэ.
  const filtered = useMemo(() => {
    ...
    return all.filter((r) => {
      if (
        sn &&
        !fold(r.nomek_25k).includes(sn) &&
        !fold(r.nomek_100k).includes(sn)
      )
        return false;
      if (
        s &&
        !cs.some((c) => fold(c.get(r)).includes(s)) &&
        !fold(r.nomek_25k).includes(s) &&
        !fold(r.nomek_100k).includes(s)
      )
        return false;
      return true;
    });
      ...
      <Card>
        ...
        >
          <TextField
            ...
            placeholder="Нэр / зураг дээрх нэрээр хайх..."
            InputProps={{
              startAdornment: (
          ...
          {/* Нэрлэвэр — тусдаа талбар. Дээрх хайлттай ЗЭРЭГ (AND) үйлчилнэ. */}
          <TextField
            value={qNomek}
            onChange={(e) => setQNomek(e.target.value)}
            placeholder="Нэрлэвэр (ж: M-47-73-Б)"
            sx={{ width: { xs: 1, sm: 300 }, flexShrink: 0 }}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <Iconify
        ...
        {/* Баримт ↔ нэр холбох хэсэг нь «Суурин судалгаа» табын
            «Баримт бичгийн холболт» карт руу шилжсэн. */}
