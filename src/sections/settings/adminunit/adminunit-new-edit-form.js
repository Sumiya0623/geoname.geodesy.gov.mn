...
export default function AdminUnitNewEditForm({
  currentConstant,
  onCloseForm,
  refetch,
  fixedParentId,
}) {
  const { constants } = useGetConstantsFordropdown("PARENT_MENUS");
  const { enqueueSnackbar } = useSnackbar();
  const NewConstantSchema = Yup.object().shape({
    unit: Yup.string().required(requiredMsg),
    centername: Yup.string().nullable(),
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        enqueueSnackbar(
          `Нэгж амжилттай ${currentConstant ? "өөрчлөгдлөө" : "нэмэгдлээ"}`
        );
        reset();
        onCloseForm();
        refetch();
      ...
      enqueueSnackbar(
        `Нэгж ${currentConstant ? "өөрчлөх" : "үүсгэх"} үед алдаа гарлаа`,
        {
          variant: "error",
        }
      );
...
AdminUnitNewEditForm.propTypes = {
